...
function init(config) {
  const store = window.MeuPlayerRedeBuzzStore;
  ...
  function goToCanaisSource() {
    ...
    window.location.href = `/canais.html?canal=${encodeURIComponent(channel.id)}`;
  }
...
window.MeuPlayerRedeBuzzUI = { init, buildEmbedUrl, normalizeChannel, extractChannels };
