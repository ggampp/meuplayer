(function () {
  if (window.MeuPlayerProviderModal) return;
  ...
  function createModalDOM() {
    ...
    backdropEl.className = 'mp-modal-backdrop';
    ...
    const closeBtn = backdropEl.querySelector('.mp-modal__close');
    const cancelBtn = backdropEl.querySelector('.mp-modal__cancel');
    const form = backdropEl.querySelector('#mpProviderForm');
    const iconInput = backdropEl.querySelector('#mpProviderIcon');
    const previewImg = backdropEl.querySelector('#mpPreviewImg');
    const presetBtns = backdropEl.querySelectorAll('.mp-modal__preset-btn');

    function updatePreview() {
      const val = iconInput.value.trim();
      previewImg.src = val || '/img/providers/default-provider.svg';
    }
    ...
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const name = backdropEl.querySelector('#mpProviderName').value.trim();
      const url = backdropEl.querySelector('#mpProviderUrl').value.trim();
      let icon = iconInput.value.trim();
      if (!icon) icon = '/img/providers/default-provider.svg';
      ...
      try {
        ...
        alert('Erro de conexão ao salvar provedor.');
      }
    });
  }
  ...
  function open() {
    ...
    const form = backdropEl.querySelector('#mpProviderForm');
    form.reset();
    backdropEl.querySelector('#mpPreviewImg').src = '/img/providers/default-provider.svg';
    backdropEl.classList.add('active');
    setTimeout(() => {
      backdropEl.querySelector('#mpProviderName').focus();
    }, 100);
  }
